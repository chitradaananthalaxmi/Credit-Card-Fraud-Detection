...
with st.sidebar:
    ...
    # KEY FIX: Read bytes immediately and store in session_state.
    # This survives the rerun triggered by clicking the button.
    if uploaded is not None:
        st.session_state['csv_bytes'] = uploaded.read()
        st.session_state['csv_name']  = uploaded.name

    # Show status from session_state (not from the widget which resets on rerun)
    if st.session_state.get('csv_bytes') is not None:
        st.markdown(f"""
        <div style="font-family:'Share Tech Mono',monospace;font-size:0.68rem;
                    color:#00ff88;background:rgba(0,255,136,0.07);
                    border:1px solid #00ff8840;border-radius:3px;
                    padding:0.5rem 0.7rem;margin-bottom:0.5rem;">
            ✓ FILE READY &nbsp;·&nbsp;
            <span style="color:#4a7fa5;">{st.session_state.get('csv_name','creditcard.csv')}</span>
...
def make_radar_fig(models_data):
    ...
    fills   = {'LR':'rgba(0,245,255,0.07)','DT':'rgba(255,230,0,0.07)','RF':'rgba(0,255,136,0.07)'}
    for name, vals in models_data.items():
        fig.add_trace(go.Scatterpolar(
            r=vals + [vals[0]], theta=cats + [cats[0]],
            fill='toself', name=name,
            line=dict(color=palette.get(name,'#fff'), width=2),
            fillcolor=fills.get(name,'rgba(255,255,255,0.05)')
        ))
    fig.update_layout(
        polar=dict(
...
with tabs[0]:
    ...
    <div class="glasscard" style="margin-top:.5rem;">
        <div style="font-family:'Share Tech Mono',monospace;font-size:0.68rem;color:#1a5a80;letter-spacing:.15em;margin-bottom:.8rem;">HOW TO USE</div>
...
if run_btn:
    ...
    if csv_bytes is None:
        st.error("⚠  No data file found. Please upload creditcard.csv in the sidebar first.")
    else:
        with st.spinner("Parsing dataset..."):
            data = load_data(csv_bytes, sample_size)

        X = data.drop('Class', axis=1)
        y = data['Class']
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, random_state=42)

        results, cms, reports, model_objs = {}, {}, {}, {}
        ...
        if use_lr:
            ...
            model_objs['LR'] = lr
            step += 1; progress_bar.progress(step / total_models)
        ...
        st.session_state.update({
            'results': results, 'cms': cms, 'reports': reports,
            'model_objs': model_objs, 'X_test': X_test,
            'y_test': y_test, 'trained': True, 'data': data
        })
    ...
    if st.session_state.get('trained'):
        ...
        for name, acc in results.items():
            fig_bar.add_trace(go.Bar(
                x=[name], y=[acc],
                marker=dict(color=palette[name], opacity=0.85, line=dict(color=palette[name], width=1)),
        ...
        fig_bar.update_layout(
            ...
            showlegend=False, margin=dict(l=20,r=20,t=40,b=20), height=320, bargap=0.4
        )
        ...
        for name, rep in reports.items():
            ...
            radar_data[name] = [results[name], fraud.get('precision',0), fraud.get('recall',0), fraud.get('f1-score',0)]
...
with tabs[2]:
    if st.session_state.get('trained'):
        ...
        for i, (name, cm) in enumerate(cms.items()):
            with cols[i]:
                st.plotly_chart(make_confusion_fig(cm, f"{name} — CONFUSION MATRIX", palette[name]), use_container_width=True)
        ...
        for name, rep in st.session_state['reports'].items():
            ...
            <div class="glasscard" style="border-color:{color}30;">
                <div style="font-family:'Orbitron',monospace;font-size:0.8rem;color:{color};letter-spacing:.15em;margin-bottom:.8rem;">{name} — REPORT</div>
                <table style="width:100%;font-family:'Share Tech Mono',monospace;font-size:0.78rem;color:#4a7fa5;border-collapse:collapse;">
                    <tr style="color:#1a5a80;font-size:0.65rem;letter-spacing:.1em;">
                        <td style="padding:.35rem .6rem;">CLASS</td><td style="padding:.35rem .6rem;">PRECISION</td>
                        <td style="padding:.35rem .6rem;">RECALL</td><td style="padding:.35rem .6rem;">F1-SCORE</td>
...
with tabs[3]:
    st.markdown("""
    <div style="font-family:'Share Tech Mono',monospace;font-size:0.7rem;color:#1a5a80;letter-spacing:.15em;margin-bottom:1rem;">
        ◉  MANUAL TRANSACTION SCAN — pick a test row and classify it
    </div>""", unsafe_allow_html=True)
        ...
        with col_right:
            ...
            if st.button("⚡ SCAN TRANSACTION"):
                ...
                if proba is not None:
                    prob_html = f"<div style='font-family:Share Tech Mono;font-size:.8rem;color:#4a7fa5;margin-top:.7rem;'>FRAUD PROB: <span style='color:#ff003c'>{proba[1]:.3f}</span> &nbsp;|&nbsp; LEGIT PROB: <span style='color:#00ff88'>{proba[0]:.3f}</span></div>"
                st.markdown(f"""
                <div class="glasscard" style="border-color:{verdict_color}50;text-align:center;padding:1.5rem;">
                    <div style="font-family:'Orbitron',monospace;font-size:1.3rem;color:{verdict_color};letter-spacing:.1em;">{verdict_label}</div>
                    {prob_html}
                    <div style="font-family:Share Tech Mono;font-size:.68rem;color:#1a4060;margin-top:.5rem;">MODEL: {model_choice}</div>
                </div>""", unsafe_allow_html=True)

        st.markdown("#### Transaction feature vector")
        st.dataframe(
            sample_row.T.rename(columns={sample_row.index[0]: "value"}).round(5),
            height=320,
...
with tabs[4]:
    if st.session_state.get('data') is not None:
        ...
        fig_pie = go.Figure(go.Pie(
            ...
            marker=dict(colors=['#00ff88','#ff003c'], line=dict(color='#020812', width=3)),
            textfont=dict(family='Orbitron', size=11), hole=0.55
        ))
        ...
        st.markdown("""
        <div class="glasscard" style="text-align:center;padding:3rem;">
            <div style="font-family:'Orbitron',monospace;font-size:0.9rem;color:#1a4060;letter-spacing:.15em;">◈ AWAITING DATA FEED</div>
        </div>""", unsafe_allow_html=True)
